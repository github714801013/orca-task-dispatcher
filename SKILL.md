...
- 后续任务分发在当前会话内完成：当前会话直接执行任务查询、Jira 需求归档、调研、路由判断与制品暂存；这些准备阶段不调用 Orca、不创建 worktree，也不通过 Orca 编排创建或打开新的 Agent 会话。工作区解析或创建、分支改名、制品迁入与受监督 worker 启动全部由已确认的 `launch` 完成；启动后仅执行下述只读启动验收。
...
1. 下发文本只使用所选流程的 `command_template` 渲染结果，不额外追加监督、心跳、汇报或追问要求；Orca 自动附带的协议前导保持原样。
2. `worker-start` 返回 `ok=true` 即确认任务分发完成，记录返回的 `dispatchId`、`taskId`、`runId`、`state` 与 `stage`；`outcome_unknown` / `turn_start_unobserved` 只表示 Orca 未观察到起步时刻，不改变已完成的投递结论。
3. 汇总“任务 / 流程 / 已分发 / Dispatch 回执”后结束本轮，不等待开发完成、不读取 worker 输出、不进入持续监督循环、不向 worker 追问；失败回执才按既有错误处理，不重发、不自动追加提示词。
